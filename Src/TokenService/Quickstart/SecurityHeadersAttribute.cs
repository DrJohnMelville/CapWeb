    ...
    public class SecurityHeadersAttribute : ActionFilterAttribute
    {
        public override void OnResultExecuting(ResultExecutingContext context)
        {
            ...
            if (result is ViewResult)
            {
                ...
                {
                    context.HttpContext.Response.Headers.Append("X-Frame-Options", "allow-from *");
                }

                // https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy
                var csp = 
                    "default-src 'self' code.jquery.com cdn.jsdelivr.net; " +
                    "object-src 'none'; " +
                    "sandbox allow-forms allow-same-origin allow-scripts; base-uri 'self';";
                // opriginal was                var csp = "default-src 'self'; object-src 'none'; frame-ancestors 'none'; sandbox allow-forms allow-same-origin allow-scripts; base-uri 'self';";
